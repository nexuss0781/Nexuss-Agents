...
export const appRouter = router({
  ...
  workspace: router({
    ...
    mission: router({
      intakePreview: publicProcedure.input(z.object({ projectId: z.string().min(1).max(128).nullable().optional(), model: z.string().trim().min(1).max(256).optional(), sources: z.array(z.object({ kind: z.enum(["raw_prompt", "plan_text", "specification"]), text: z.string().trim().min(1).max(120_000), name: z.string().trim().max(240).optional(), mimeType: z.string().trim().max(120).optional() })).min(1).max(20) })).mutation(async ({ ctx, input }) => missionCall(async () => runMissionIntake(await workspaceOwner(ctx), input))),
      createFromIntake: publicProcedure.input(z.object({ projectId: z.string().min(1).max(128).nullable().optional(), model: z.string().trim().min(1).max(256).optional(), sources: z.array(z.object({ kind: z.enum(["raw_prompt", "plan_text", "specification"]), text: z.string().trim().min(1).max(120_000), name: z.string().trim().max(240).optional(), mimeType: z.string().trim().max(120).optional() })).min(1).max(20), budget: z.object({ maxDepth: z.number().int().min(1).max(10), maxChildWorkItems: z.number().int().min(1).max(1_000), maxAgentAttempts: z.number().int().min(1).max(20), maxToolCalls: z.number().int().min(1).max(10_000), maxModelTokens: z.number().int().min(1_000).max(10_000_000), maxDurationSeconds: z.number().int().min(1).max(86_400) }).optional() })).mutation(async ({ ctx, input }) => missionCall(async () => createMissionFromIntake(await workspaceOwner(ctx), input))),
