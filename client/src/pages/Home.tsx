...
export default function Home({ profileName = "Nexuss Operator", profileEmail, profileAvatarUrl, onSignOut, signOutPending = false }: HomeProps) {
  ...
  const githubBranchesQuery = trpc.workspace.github.branches.useQuery({ fullName: selectedGithubRepository?.fullName || "placeholder/repository" }, { enabled: Boolean(projectEditor?.mode === "create" && projectTab === "github" && repositoryTab === "import" && selectedGithubRepository?.fullName), retry: false, staleTime: 60_000 });
  ...
  function selectAuthorizedRepository(repository: GithubRepository) {
    ...
    setSelectedGithubBranch(repository.defaultBranch || "");
    setProjectGithubUrl("");
