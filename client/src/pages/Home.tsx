  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
        ...
        <div className="sidebar-footer"><button className="footer-action" onClick={() => toast("Authentication is not enabled in this build") }><Settings2 size={15} /> Preferences <span>⌘ ,</span></button></div>
      </aside>

      <main className="main-stage">
        <header className="topbar"><div className="topbar-left"><button className="icon-button mobile-menu" onClick={() => setMobileNav(true)} aria-label="Open navigation"><Menu size={19} /></button><div className="breadcrumb"><span>Workspace</span><ChevronDown size={14} /><span className="breadcrumb-muted">{activeProject?.name || "Unassigned"}</span></div><div className="topbar-thread-title">{activeThread?.title || "New thread"}</div></div><div className="topbar-right"><span className="mode-label">DARK MODE</span><span className="topbar-rule" /><button className="icon-button" onClick={() => toast("Command palette coming soon")} aria-label="Command palette"><Command size={17} /></button><div className="avatar">NA</div></div></header>
