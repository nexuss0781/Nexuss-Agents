...
export default function NexussGitChangesPanel({ repositories, selectedRepository, currentProject, onSelect }: Props) {
  ...
  if (!activeRepository) return <div className="nexuss-git-changes-empty"><span><RefreshCw size={22} /></span><h2>Loading project repository</h2><p>The linked repository <strong>{linkedFullName}</strong> is not available in the current GitHub connection. Refresh or reconnect GitHub, then retry.</p></div>;
